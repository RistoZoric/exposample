<nav class="top-0 z-10 text-md font-semibold w-full"
    :class="{'static bg-transparent text-glacier-500 pt-8 pb-4': !scrolledFromTop, 'fixed bg-white text-chantilly-500 py-6 shadow-lg': scrolledFromTop}">
    <x-ui.container>
        <div class="flex flex-row justify-between items-center">
            <a href="{{ route('pages.home') }}">
                <img src="{{ asset('images/logo/gradient-lg.png') }}" class="h-8" alt="Privy logo" />
            </a>
            @if (!Route::is('pages.home') && !Route::is('pages.featured_creators'))
            <div class="hidden lg:flex flex items-center justify-center px-4">
                <form action="/categories/search?q=" method="GET" role="search">
                <div class="flex border rounded" :class="!scrolledFromTop ? 'border-white' : 'border-black'">
                    <input type="text" :class="!scrolledFromTop ? 'px-4 py-2 text-white bg-transparent placeholder-white hover:bg-white hover:text-black active:bg-white' : 'px-4 py-2 hover:bg-white hover:text-black active:bg-white'" placeholder="Search..." value="" name="q">
                    <button class="flex items-center justify-center px-4">
                        <svg :class="!scrolledFromTop ? 'w-6 h-6 text-white bg-transparent hover:text-black' : 'w-6 h-6 text-black hover:text-black'" fill="currentColor" xmlns="http://www.w3.org/2000/svg"
                            viewBox="0 0 24 24">
                            <path
            ...
            <div class="hidden lg:flex flex-row items-center space-x-8">
                ...
                @else
                    <a href="{{ route('pages.creators') }}" class="py-[4px] px-12 rounded-2xl"
                        :class="{'bg-glacier-500 text-white': !scrolledFromTop, 'bg-chantilly-500 text-white': scrolledFromTop}">
            ...
            <div
                ...
                :class="{'text-glacier-500 border-glacier-500': !scrolledFromTop, 'text-chantilly-500 border-chantilly-500': scrolledFromTop}"
            ...
            >
                <div class="flex flex-col items-start space-y-8">
                    ...
                    <div class="flex flex-row justify-between items-center w-full">
                        <form action="/categories/search?q=" method="GET" role="search">
                            <div class="flex border border-white rounded">
                                <input type="text" class="px-4 py-2 w-40 text-white bg-transparent placeholder-white hover:bg-white hover:text-black active:bg-white" placeholder="Search..." value="" name="q">
                                <button class="flex items-center justify-center px-4">
